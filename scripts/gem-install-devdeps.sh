...
_LOCK_OPT=""
if [[ "$($BR_GEM_CMD --version)" == *"2.4"* ]]; then
    _LOCK_OPT+=" --no-lock"
fi
...
if [ $BR_ISOLATE == "yes" ]; then
    ...
    $BR_GEM_CMD install -g $BR_DEV_DEPS $BR_GEM_INSTALL_OPTS $_LOCK_OPT --no-user-install || exit 1
    # Just say no to lock files (cleanup if we're using an old gem version)
    rm -f "$BR_DEV_DEPS".lock
else
    echo "Installing development dependencies."
    $BR_GEM_CMD install -g $BR_DEV_DEPS $BR_GEM_INSTALL_OPTS $_LOCK_OPT || exit 1
fi
